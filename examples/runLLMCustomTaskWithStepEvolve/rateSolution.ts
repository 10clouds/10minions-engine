...
export async function rateSolution(
  ...
  const rawResult = await gptExecute({
    fullPrompt: createFullPromptFromSections({
      intro:
        'Rate how good is the following SOLUTION to the PROBLEM described below.',
      sections: {
        PROBLEM: task.task,
        ...
        CRITERIA: shuffleArray(criteriaDefinition.slice())
          ...
          .map(
            (c) => `${c.name} - Max of ${c.maxPoints}pts if ${c.maxPointsIf}`,
          )
          .join('\n'),
      },
    }),
  ...
  const results = [
    ...criteriaDefinition
      ...
      .map((c) => {
        return {
          rating:
            c.calculate !== 'GPT' ? c.calculate(solutionWithMeta.solution) : 0,
          criteria: c.name,
          reasoning: '',
        };
      }),
