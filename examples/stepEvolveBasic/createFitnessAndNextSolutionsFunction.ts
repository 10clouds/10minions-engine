...
export function createFitnessAndNextSolutionsFunction(
  ...
  const fitnessAndNextSolutionsFunction = async (
    ...
    const fitness = task.avoidThoseNumbers.reduce(
      (fitness, avoidThisNumber) => {
        return (
          fitness -
          1 / (0.1 + Math.abs(solutionWithMeta.solution - avoidThisNumber))
        );
      },
      0,
    );
    ...
    return {
      ...
      fitnessComponents: [{ id: 'fitness', fitness }],
