...
import { fuzzyReplaceText } from '../../src/utils/code/fuzzyReplaceText';

suite('Replace Test Suite', () => {
  test('Basic test case: Exact match found', async () => {
    const currentCode = [`      function example() {`, `        console.log("Hello, world!");`, `      }`].join('\n');

    const findText = [`console.log("Hello, world!");`].join('\n');

    const withText = [`console.log("Hello, moon!");`].join('\n');

    const expectedOutput = [`      function example() {`, `        console.log("Hello, moon!");`, `      }`].join('\n');

    const result = await fuzzyReplaceText({ currentCode, findText, withText });
    assert.strictEqual(result, expectedOutput);
  });
  test('Only replace with same indentation', async () => {
    const currentCode = [`      function example() {`, `        console.log("Hello, world!");`, `      }`, `      console.log("Hello, world!");`].join('\n');

    const findText = [`      console.log("Hello, world!");`].join('\n');

    const withText = [`      console.log("Hello, moon!");`].join('\n');

    const expectedOutput = [`      function example() {`, `        console.log("Hello, world!");`, `      }`, `      console.log("Hello, moon!");`].join('\n');
  ...
  test('Only replace with same indentation (multiline)', async () => {
    ...
    const findText = [`      console.log("Hello, world!");`, `      console.log("Hello, warg!");`].join('\n');
  ...
  test('Multiline with different indentation', async () => {
    ...
    const expectedOutput = [`      function example() {`, `        if (true) {`, `          console.log("Hello, moon!");`, `        }`, `      }`].join('\n');
  ...
  test('Fuzzy match with different syntax', async () => {
    const currentCode = [`      function example() {`, `        console.log(say_hello("world"));`, `      }`].join('\n');

    const findText = [`      console.log(say_hello("world"));`].join('\n');

    const withText = [`      console.log(say_hello("moon"));`].join('\n');

    const expectedOutput = [`      function example() {`, `        console.log(say_hello("moon"));`, `      }`].join('\n');

    const result = await fuzzyReplaceText({ currentCode, findText, withText });
    assert.strictEqual(result, expectedOutput);
  });
  test('No match found', async () => {
    const currentCode = [`      function example() {`, `        handleClearAndFocus();`, `      }`].join('\n');
